$(function(){
        ...
        $.ajax({
            ...
            success: function(response){
                if(response.ok==0)
                {
                    ...
                    {
                        if(response.orders[i].postId == username){
                            $('#home').append('<div class="smallCard"><p><strong>接单人：</strong>'+ response.orders[i].takeId + '</p><p><strong>描述：</strong>'+ response.orders[i].description +'</p><p><strong>金钱：</strong>'+ response.orders[i].money +'元</p><p> <strong>时间：</strong>'+ response.orders[i].time +'以前</p></div>');
                            $('.smallCard').attr('data-index', response.orders[i].orderId);
                            $('.smallCard').on('click',function(){
                                var temp = this;
                                if(temp.isTake == 1 && temp.isDone==0)
                                {
                                    $('.container').append('<div id="hide"><div id="content"></div><div id="bttn"><button id="close">关闭</button><button id="confirm">未完成</button></div></div>');
                                
                                    var text = $(temp).html();
                                    $('#content').html(text);
                                    
                                    $('#hide').css("display","block");
                                    var params = {orderId : $(temp).attr('data-index'), isDone : 1};
                                ...
                                else if(temp.isTake == 1 && temp.isDone == 1)
                                {
    ...
    $('#hint').on('click',function(){
        ...
        $.get("SearchMyPostOrder", function(response){
